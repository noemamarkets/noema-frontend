...
// User name storage
let userName = localStorage.getItem('noema_user_name') || null;
...
function saveNameAndStart() {
  ...
  if (name) {
    localStorage.setItem('noema_user_name', name);
    userName = name;
...
function getGreeting() {
  ...
  const greetings = {
    morning: userName ? `Good morning, ${userName}` : "Good morning",
    afternoon: userName ? `Good afternoon, ${userName}` : "Good afternoon",  
    evening: userName ? `Good evening, ${userName}` : "Good evening"
  };
...
// Load watchlist with smart DOM updates (no rebuilding on refresh)
...
async function loadWatchlist() {
  ...
  if (isFirstLoad) {
    container.innerHTML = quotes.map(quote => {
      ...
      return `
        <div class="stock-card ${cardClass}" data-symbol="${quote.symbol}" onclick="removeStock('${quote.symbol}')">
    ...
    // Subsequent loads: Just update the numbers (no DOM rebuild!)
    quotes.forEach(quote => {
      const card = container.querySelector(`[data-symbol="${quote.symbol}"]`);
      ...
      // Update card class (for top border color)
      card.className = `stock-card ${cardClass}`;
      card.setAttribute('onclick', `removeStock('${quote.symbol}')`);
      ...
      if (changeElement) {
        changeElement.className = `stock-change ${changeColor}`;
        changeElement.textContent = `${isPositive ? '+' : ''}${quote.change.toFixed(2)} (${isPositive ? '+' : ''}${quote.changePercent.toFixed(2)}%)`;
      }
    });
  }
}
