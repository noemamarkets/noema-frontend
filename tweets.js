...
const DISPLAY_DURATION = 30000; // 30 seconds per tweet

// Load and display tweets with auto-scroll
function loadTweets() {
  const pulseSection = document.getElementById('pulse-section');
  
  if (!pulseSection) return;
  
  // Show all tweets initially
  renderAllTweets();
  
  // Start auto-scroll after first tweet has been displayed
  setTimeout(() => {
    startAutoScroll();
  }, DISPLAY_DURATION);
}

// Render all tweets (for initial load)
function renderAllTweets() {
  ...
  pulseSection.innerHTML = tweets.map((tweet, index) => {
    ...
    return `
      <div class="pulse-tweet ${cardClass}" data-tweet-index="${index}" style="padding: 20px; border-radius: 12px; transition: all 0.5s ease;">
        ${featuredBadge}
        <a href="${tweet.url}" 
           target="_blank" 
...
// Start auto-scrolling through tweets
...
function scrollToNextTweet() {
  ...
  // Hide current tweet
  tweetElements[currentTweetIndex].style.opacity = '0';
  tweetElements[currentTweetIndex].style.transform = 'translateY(-20px)';
  
  // Move to next tweet (loop back to start)
  currentTweetIndex = (currentTweetIndex + 1) % tweets.length;
  
  // Show next tweet after brief delay
  setTimeout(() => {
    // Scroll to bring tweet into view
    tweetElements[currentTweetIndex].scrollIntoView({ 
      behavior: 'smooth', 
      block: 'nearest' 
    });
    
    // Fade in next tweet
    tweetElements[currentTweetIndex].style.opacity = '1';
    tweetElements[currentTweetIndex].style.transform = 'translateY(0)';
  }, 300);
}
...
function pauseOnHover() {
  ...
  if (pulseSection) {
    pulseSection.addEventListener('mouseenter', () => {
      if (autoScrollInterval) {
        clearInterval(autoScrollInterval);
      }
    });
    
    pulseSection.addEventListener('mouseleave', () => {
      startAutoScroll();
    });
  }
}
